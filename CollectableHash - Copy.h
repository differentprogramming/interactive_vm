...
struct HashTable :public Collectable
{
	...
	int used;
	mutable int wasted;
	RootPtr < CollectableInlineVector<HashEntry<K, V>>> data;
	V empty_v;


	HashTable(const V& ev,int s = INITIAL_HASH_SIZE) :HASH_SIZE(nearest_power_of_2(s)), used(0), wasted(0), data(new CollectableInlineVector<HashEntry<K, V>>(HASH_SIZE)),empty_v(ev) {}
	...
	void inc_used()
	{
		...
		if (((used + wasted) << 2) > HASH_SIZE)
		{
			int OLD_HASH_SIZE = HASH_SIZE;
			HASH_SIZE <<= 1;
			RootPtr<CollectableInlineVector<HashEntry<K, V> > > t(data);
			data = new CollectableInlineVector<HashEntry<K, V> >(HASH_SIZE);			
			used = 0;
			wasted = 0;
			for (int i = 0; i < OLD_HASH_SIZE; ++i) {
				GC::safe_point();
